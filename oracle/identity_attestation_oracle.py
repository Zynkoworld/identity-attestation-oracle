#!/usr/bin/env python3
"""identity-attestation-oracle -- a deterministic attestation-integrity decider for the `identity` knowledge domain.

An **oracle** *deterministically decides* the truth of a case. This one decides, for a `identity` knowledge
capsule, whether it is a well-formed, provenance-carrying attestation. VALID iff: domain=='identity', a non-empty
...
def decide(c):
    if c.get("domain") != "identity": return "INVALID"
...
def _run_probes(path):
    ...
    print("identity attestation oracle: probes=%d | recall=%.3f | false_positives=%d | verdict=%s"%(len(probes),recall,fp,"PASS" if ok else "FAIL"))
